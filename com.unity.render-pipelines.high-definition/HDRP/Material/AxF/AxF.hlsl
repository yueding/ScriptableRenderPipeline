...
struct PreLightData {
    ...
    float3  IBLDominantDirectionWS_ClearCoat;   // Dominant specular direction, used for IBL in EvaluateBSDF_Env() and also in area lights when clear coat is enabled
    #ifdef _AXF_BRDF_TYPE_SVBRDF
                        ...
                        out float3 diffuseLighting, out float3 specularLighting)
{
    ...
    GetScreenSpaceAmbientOcclusionMultibounce(  posInput.positionSS, preLightData.viewWS_UnderCoat, RoughnessToPerceptualRoughness(0.5 * (bsdfData.roughness.x + bsdfData.roughness.y)),
